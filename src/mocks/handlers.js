...
export const handlers = [
	...
	rest.post(`${urlBase}/login`, (req, res, ctx) => {
		...
		if (username === 'Lambda School' && password === 'i<3Lambd4') {
			...
			return res(ctx.status(403), ctx.json({ error: 'Username or Password incorrect. Please see Readme' }));
		}
	}),
	...
	rest.get(`${urlBase}/colors`, (req, res, ctx) => {
		if (authenticator(req)) {
			...
			res(ctx.status(403), ctx.json({ error: 'User must be logged in to do that.' }));
		}
	}),
